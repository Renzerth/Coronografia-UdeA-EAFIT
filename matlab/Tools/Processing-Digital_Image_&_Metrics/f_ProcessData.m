...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%% Temporal reference images loading
...
Lyotimg = rgb2gray(Lyotimg);                                                                                               % TO DELETE
...
refmeas = PSFimg;                                                                                                                 % TO DELETE
[xangL_D,yangL_D,aproxCenter,aproxRadius] = f_approximateSpotSize(refmeas);
...
%%% OLD: center of the image but not of the spot
...
% halfX = round((maxX+1)/2); % x mid point
% halfY = round((maxY+1)/2); % y mid point
% Old: halfX = xSize/2; halfY = ySize/2;
...
%% Symmetric pixels
% ORIGINAL
% xpix = -halfX : halfX - 1; % pixels
% ypix= -halfY :  halfY - 1; % pixels                   
...
[x,y,HprofRef,VprofRef,~,~] = f_makeImageProfile(xpix,ypix,midX,midY, ...
           refMeas,tol,shiftCart,tit,xlab,ylab,plotData,plotH,plotV,oneSideProfile);

%% Reference profile choice
...
[xpix,ypix,Hprofmeas,Vprofmeas,~,~] = f_makeImageProfile(xpix,ypix,midX,midY, ...
   expMeas{idxgral},tol,shiftCart,tit,xlab,ylab,plotData,plotH,plotV,oneSideProfile);
