...
function z = f_LaguerreGaussianE(params,r,varargin)
...
if nargin>=4                
    if ischar(varargin{2}) && strcmp(varargin{2},'cart')
...
if min(size(r))>=2
    for u=1:size(params,1)
            z(:,:,u) = a(u)...
                ...
                .* (sqrt(2)*r/w(u)).^m(u) .*exp(1i*signm(u)*m(u).*theta).* f_LaguerrePoly([p(u),m(u)],2*r.^2/w(u).^2)...
