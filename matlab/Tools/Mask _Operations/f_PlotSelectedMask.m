...
r,phi,phaseValues,tc,s,ph0,p,WsizeRatio,L,f_FR,bcst,period,T0, ...
...
switch maskSel 
 case 0 % Spiral phase mask or mapa de fase espiral or mascara espiral
        % de fase or mascara helicoidal de fase 
  [mask,wrapMask,wrapMaskFig] = f_SpiralMask(r,phi,phaseValues,tc,s,ph0,...
  normMag,binMask,binv,MaskPupil,rSize,monitorSize,scrnIdx,coordType, ...
  abs_ang,MaxMask,plotMask);
  maskName = 'Spiral';
   
 case 1 % Laguerre-Gauss (LG) beams
  [mask,wrapMask,wrapMaskFig] = f_LGMask(r,phi,phaseValues,tc,s,ph0,p, ...
  WsizeRatio,normMag,binMask,binv,MaskPupil,rSize,monitorSize,scrnIdx, ...
  coordType,abs_ang,MaxMask,plotMask);
  maskName = 'LG';
 
 case 2 % Vortex Producing Lens (VPL) = Helicoidal + Fresnel lens
  [mask,wrapMask,wrapMaskFig] = f_VPLMask(r,phi,phaseValues,tc,s, ...
 ...
 case 3 % Elliptic Gaussian Vortex (EGV) mask or Vortice eliptico-gaussiano
  [mask,wrapMask,wrapMaskFig] = f_EGVMask(X,Y,r,phaseValues,tc,s,ph0, ...
  bcst,normMag,binMask,binv,MaskPupil,rSize,monitorSize,scrnIdx, ...
  coordType,abs_ang,MaxMask,plotMask);
  maskName = 'EGV';
 ...
 case 4 % Threefold dislocation hologram or double pitch fork hologram or
        ...
        % grating plate or rejilla de Ronchi con una dislocacion 
        % (2013_Vortex_Generations_Mach-Zehnder_Interferometer)
  [mask,wrapMask,wrapMaskFig] = f_ForkMask(X,Y,r,phi,phaseValues,tc, ...
...
%%%%%%%%%%%%%%%%%%%% NOT USED BUT FOR ACADEMIC PURPOSES %%%%%%%%%%%%%%%%%%%
 case 5 % Zernike (aberrations)
  [mask,wrapMask,wrapMaskFig,~] = f_ZernikeMask(X,Y,r,z_coeff,z_a,L, ...
 ...
 case 6 % Laguerre-Gauss (LG) + Zernike
  [mask,wrapMask,wrapMaskFig] = f_LGZernikeMask(X,Y,r,phi,phaseValues, ...
  tc,s,ph0,p,WsizeRatio,z_coeff,z_a,L,z_pupil,z_disp_wrap,z_plot, ...
  normMag,binMask,binv,rSize,monitorSize,scrnIdx,coordType,abs_ang, ...
  MaxMask,plotMask);
  maskName = 'LG_Zernike';       
