...
frkTyp,Aalpha,Angalp,Angbet,z_coeff,z_a,z_frac,z_pupil,z_disp_wrap,...
z_plot, normMag,binMask,binv,MaskPupil,rSize,monitorSize,scrnIdx,coordType,abs_ang, ...
...
switch maskSel 
 case 0 % Spiral phase mask or mapa de fase espiral or máscara espiral
  ...
  levShft,tc,s,ph0,normMag,binMask,binv,MaskPupil,rSize,monitorSize,scrnIdx,coordType, ...
  abs_ang,MaxMask,plotMask);
  maskName = 'Spiral';
   
 case 1 % Laguerre-Gauss (LG) beams
  [mask,wrapMask,wrapMaskFig] = f_LGMask(r,phi,gl,phaseValues,mingl,maxgl, ...
  levShft,tc,s,ph0,p,WsizeRatio,normMag,binMask,binv,MaskPupil,rSize,monitorSize,scrnIdx, ...
  coordType,abs_ang,MaxMask,plotMask);
  maskName = 'LG';
 
 case 2 % Vortex Producing Lens (VPL) = Helicoidal + Fresnel lens
  [mask,wrapMask,wrapMaskFig] = f_VPLMask(r,phi,gl,phaseValues,mingl,maxgl, ...
  levShft,tc,s,ph0,L,f_FR,normMag,binMask,binv,MaskPupil,rSize,monitorSize,scrnIdx, ...
  coordType,abs_ang,MaxMask,plotMask);
  maskName = 'VPL';
  
 case 3 % Elliptic Gaussian Vortex (EGV) mask or Vórtice elíptico-gaussiano
  [mask,wrapMask,wrapMaskFig] = f_EGVMask(X,Y,r,gl,phaseValues,mingl,maxgl, ...
  levShft,tc,s,ph0,bcst,normMag,binMask,binv,MaskPupil,rSize,monitorSize,scrnIdx, ...
  coordType,abs_ang,MaxMask,plotMask);
  maskName = 'EGV';
 ...
 case 4 % Threefold dislocation hologram or double pitch fork hologram or
  ...
  [mask,wrapMask,wrapMaskFig] = f_ForkMask(X,Y,r,phi,gl,phaseValues,mingl, ...
  maxgl,levShft,tc,s,ph0,L,period,T0,frkTyp,Aalpha,Angalp,Angbet, ...
  normMag,binMask,binv,MaskPupil,rSize,monitorSize,scrnIdx,coordType,abs_ang,MaxMask, ...
  plotMask);
  maskName = 'Fork';
  
%%%%%%%%%%%%%%%%%%%% NOT USED BUT FOR ACADEMIC PURPOSES %%%%%%%%%%%%%%%%%%%
 case 5 % Zernike (aberrations)
  [mask,wrapMask,wrapMaskFig] = f_ZernikeMask(X,Y,r,z_coeff,z_a,z_frac,L,gl, ...
  phaseValues,mingl,maxgl,levShft,z_pupil, ...
  z_disp_wrap,z_plot,normMag,binMask,binv,rSize,monitorSize,scrnIdx,coordType,MaxMask,plotMask);
  maskName = 'Zernike';
                  
 case 6 % Laguerre-Gauss (LG) + Zernike
  [mask,wrapMask,wrapMaskFig] = f_LGZernikeMask(X,Y,r,phi,gl,phaseValues,mingl, ...
  maxgl,levShft,tc,s,ph0,p,WsizeRatio,z_coeff,z_a,z_frac,L,z_pupil, ...
  z_disp_wrap,z_plot,normMag,binMask,binv,rSize,monitorSize,scrnIdx,coordType,abs_ang, ...
  MaxMask,plotMask);
  maskName = 'LG_Zernike';       
