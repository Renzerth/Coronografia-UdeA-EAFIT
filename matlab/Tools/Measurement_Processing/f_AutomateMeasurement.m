...
phiPC,phaseValues,mingl,maxgl,levShft,s,ph0,p,W,L,f_FR,bcst,period,T0,frkTyp,...
Aalpha,Angalp,Angbet,z_coeff,a,frac,pupil,ZernikeSize,disp_wrap,plot_z, ...
normMag,binMask,binv,monitorSize,scrnIdx,coordType,abs_ang,MaxMask, ...
maskSel,ltcvect,lglvect,wait,DatalogDir,dataformat,pathSep,cameraPlane,...
...
for idxtc = 1:ltcvect 
  for idxgl = 1:lglvect
    ...
    [X,Y,r,phi] = f_SelectCoordinates(Xslm,Yslm,rSLM,phiSLM,Xpc,Ypc,rPC,...
                                      phiPC,plotMask);
    [~,wrapMask,slmfig] = f_PlotSelectedMask(X,Y,r,phi,gl,phaseValues, ...
...
% Author: PhD student Jens de Pelsmaeker VUB B-PHOT 2018, Brussels, Belgium
    ...
    camfig = figure('units','normalized','position',[1/10 1/10 1/3 1/2]);
    ...
    phaseValues,mingl,maxgl,levShft,tc,s,ph0,p,W,L,f_FR,bcst,period,T0,frkTyp,...
    Aalpha,Angalp,Angbet,z_coeff,a,frac,pupil,ZernikeSize,disp_wrap,plot_z, ...
