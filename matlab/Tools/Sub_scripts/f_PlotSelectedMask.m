function [mask,maskName] = f_PlotSelectedMask(Xslm,Yslm,rSLM,phiSLM,Xpc,Ypc,rPC, ...
phiPC,gl,glphi,mingl,maxgl,levShft,tc,s,ph0,p,W,normLG,L,f_FR,bcst, ...
z_coeff,a,frac,pupil,sSize,disp_wrap,plot_z,binMask,binv,monitorSize, ...
...
switch maskSel 
 ...
 case 0 % Spiral phase mask or mapa de fase espiral or máscara espiral
  ...
  levShft,tc,s,ph0,binMask,binv,monitorSize,scrnIdx,coordType,abs_ang, ...
  plotMask);
  maskName = 'Spiral';
   
 case 1 % Laguerre-Gauss (LG) beams
  mask = f_LGMask(rSLM,rPC,phiSLM,phiPC,gl,glphi,mingl,maxgl,levShft, ...
  tc,s,ph0,p,W,normLG,binMask,binv,monitorSize,scrnIdx,coordType, ...
  abs_ang,plotMask);
  maskName = 'LG';
 ...
 case 2 % Vortex Producing Lens (VPL) = Helicoidal + Fresnel lens
  mask = f_VPLMask(rSLM,phiSLM,rPC,phiPC,gl,glphi,mingl,maxgl,levShft, ...
                   tc,s,ph0,L,f_FR,binMask,binv,coordType,abs_ang,showM);
  maskName = 'VPL';
  
 case 3 % Elliptic Gaussian Vortex (EGV) mask or Vórtice elíptico-gaussiano
  mask = f_EGVMask(Xslm,Yslm,Xpc,Ypc,rSLM,rPC,gl,glphi,mingl,maxgl,levShft, ...
                   tc,s,ph0,bcst,binMask,binv,coordType,abs_ang,showM);
  maskName = 'EGV';
 ...
 case 4 % Threefold dislocation hologram or double pitch fork hologram or
  ...
  binMask,binv,coordType,abs_ang,showM);
  maskName = 'Fork';
...
%%%%%%%%%%%%%%%%%%%% NOT USED BUT FOR ACADEMIC PURPOSES %%%%%%%%%%%%%%%%%%%
 case 5 % Zernike (aberrations)
  ...
  levShft,pupil,sSize,disp_wrap,plot_z,binMask,binv,monitorSize,scrnIdx, ...
  coordType,abs_ang,showM);
  maskName = 'Zernike';
                  
 case 6 % Laguerre-Gauss (LG) + Zernike
  mask = f_LGZernikeMask(rSLM,phiSLM,rPC,phiPC,gl,glphi,mingl,maxgl, ...
         levShft,tc,s,ph0,p,W,binv,normLG,z_coeff,a,frac,L,pupil,sSize, ...
         disp_wrap,plot_z,binMask,binv,monitorSize,coordType,abs_ang,showM);
  maskName = 'LG_Zernike';                    
