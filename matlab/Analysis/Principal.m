...
if meas
  ...
  if measDebug == 1 && measSimulated == 0
    [~,circShiftX,circShiftY] = f_CaptureImage(vidMeas,dataDir,filename,imgformat,pathSep, ...
    infoDelim,dirDelim,snapsfldr,previewBool,loghist,camera, ...
    cameraPlane,exposure,format,fps,maskFig,plotMask);
    if circShiftX ~= 0 || circShiftY ~= 0 % doesn't change if it didn't change at all
      [coorRelshiftX, coorRelshiftY] = f_shiftToCart(circShiftX,circShiftY,monitorSize(1),monitorSize(2));
      [shiftX,shiftY] = f_calcHScoorToSgnCoor((coorRelshiftX + monitorSize(1)/2)/monitorSize(1), ...
        (coorRelshiftY + monitorSize(2)/2)/monitorSize(2));
...
if proc == 1% && measSim == 0
  ...
  AiryFactor,metricSel,metricProfile,beepSound,L,NA,PP, ...
