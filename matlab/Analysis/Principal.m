...
f_DefineSpace(sSupport,sSize,shiftCart,shiftBool,pixSize, ...
...
[mask,wrapMask,~,maskName] = f_PlotSelectedMask(X,Y,r,phi,gl, ...
phaseValues,tc,s,ph0,p,WsizeRatio,L,f_FR,bcst,period,T0,frkTyp,Aalpha, ...
Angalp,Angbet,z_coeff,z_a,z_frac,z_pupil,z_disp_wrap,z_plot,normMag, ...
...
if meas == 1
 ...
 if measDebug == 1 && measSimulated == 0
  ...
  phiPC,phaseValues,s,ph0,p,WsizeRatio,L,f_FR,bcst,period,T0,frkTyp, ...
  Aalpha,Angalp,Angbet,z_coeff,z_a,z_frac,z_pupil,z_disp_wrap,z_plot, ...
  normMag,binMask,binv,MaskPupil,monitorSize,scrnIdx,coordType,abs_ang, ...
  MaxMask,maskSel,ltcvect,lglvect,wait,DatalogDir,dataformat,pathSep, ...
