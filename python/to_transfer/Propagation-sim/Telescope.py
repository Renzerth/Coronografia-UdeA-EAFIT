...
AS = np.zeros((M,M))
lim1=int(M/2-(int(R2/dx)+10)) #Number of pixels needed to reach R2 plus a 
lim2=int(M/2+(int(R2/dx)+10)) # little extra-range, centered in M/2 (This process is made to avoid big calculations with the whole MxM matrix)
...
ax.set_xlabel("$N_{x}$",labelpad=8)
ax.set_ylabel("$N_{y}$") 
plt.imshow((abs(E0)**2)[int(M/2-800):int(M/2+800),int(M/2-800):int(M/2+800)],cmap='gray')
plt.show()
...
ax.set_xlabel("$N_{x}$",labelpad=8)
ax.set_ylabel("$N_{y}$") 
plt.imshow((abs(Ef2)**2)[int(M/2-148):int(M/2+148),int(M/2-148):int(M/2+148)],cmap='gray')
cb=plt.colorbar()
...
ax0.set_ylabel("$N_{y}$") 
map0=ax0.imshow(FaseEv[int(M/2-120):int(M/2+120),int(M/2-120):int(M/2+120)],cmap='gray')
